...
export class PerformanceMonitor {
  ...
  private calculateResponseTimeMetrics(): void {
    if (this.responseTimeHistory.length === 0) return;
      ...
      // Replace lines 92-93 in PerformanceMonitor.ts
    ...
    const p99Index = Math.floor(durations.length * 0.99);
      ...
      if (durations.length > 0) {
        this.metrics.p95ResponseTime = durations[Math.min(p95Index, durations.length - 1)] || 0;
        this.metrics.p99ResponseTime = durations[Math.min(p99Index, durations.length - 1)] || 0;
      }
  }
