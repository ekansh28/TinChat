...
const socketToAuthId = {};
const authIdToSocketId = {};
const waitingUsers = {
    text: [],
    video: [],
};
...
const FindPartnerPayloadSchema = zod_1.z.object({
    ...
    authId: zod_1.z.string().uuid().nullable().optional().default(null),
});
...
const SendMessagePayloadSchema = RoomIdPayloadSchema.extend({
    ...
    username: zod_1.z.string().max(30).nullable().optional(),
    authId: zod_1.z.string().uuid().nullable().optional(),
});
const WebRTCSignalPayloadSchema = RoomIdPayloadSchema.extend({
    signalData: zod_1.z.any(),
});
const UpdateStatusPayloadSchema = zod_1.z.object({
    status: zod_1.z.enum(['online', 'idle', 'dnd', 'offline']),
});
// Helper function to log queue state
function logQueueState(chatType, context) {
    const queue = waitingUsers[chatType];
    console.log(`[QUEUE_STATE_${context.toUpperCase()}] ${chatType} queue (size: ${queue.length}):`, queue.map(u => ({
        socketId: u.id,
...
async function updateUserStatus(authId, status) {
    if (!supabase || !authId)
        return;
    try {
